--  Mock up in DuckDb
create
or replace view census_income_2022 as
SELECT
	right (geo_id, 5) zip_code,
	est_households_total,
	est_households_med_income,
	try_cast(
		regexp_replace(
			regexp_replace(est_households_med_income, '[^0-9]+', ''),
			'\+|\-',
			''
		) as bigint
	) est_households_med_income_,
	est_households_mean_income,
	try_cast(
		regexp_replace(
			regexp_replace(est_households_mean_income, '[^0-9]+', ''),
			'\+|\-',
			''
		) as bigint
	) est_households_mean_income_
FROM
	read_parquet(
		'D:\data-engineering-zoomcamp2024-project\data\census_income_2022.parquet'
	);
...
where
	est_households_total > 0 --	and est_households_med_income like '%,%'
order by
	est_households_total desc;

create
or replace view preparer2024 as
SELECT
...
FROM
	...
	join census_income_2022 ce on ce.zip_code = pr.zip_code
WHERE
	pr.state_code = 'CA'
GROUP BY
	pr.zip_code,
