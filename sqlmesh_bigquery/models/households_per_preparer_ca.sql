MODEL (
    name preparer_dataset.households_per_preparer_ca,
    kind VIEW
);
...
FROM
    preparer_dataset.preparers_by_zip_ca pr
    join preparer_dataset.census_income_ ce on ce.zip_code = pr.zip_code
WHERE
    pr.state_code = 'CA'
order by
